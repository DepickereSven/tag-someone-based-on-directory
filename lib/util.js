 ...
 *
 * @param config The config read from the tag-someone-config.yml file
 * @returns {string} that contains the regex path
 */
function getRegexPath(config) {
  return config['regexPath'];
}

/**
 * This will return the file status we want to tag a person for
 * IF a file is not in one of the file status we won't tag the person for a review
 * @param config The config read from the tag-someone-config.yml file
 * @returns {string} that contains the file status
 */
function getFileStatusThatAreValidForComments(config) {
  let fileStatuses = config['fileStatus'];
  if (fileStatuses === undefined) {
    fileStatuses = ['added', 'modified', 'changed'];
  }
  return fileStatuses;
}
...
module.exports = {
  ...
  getPagingNumber,
  getRegexPath,
  getFileStatusThatAreValidForComments,
  botSuffix
}
